...
packages <- c("rpart","aplpack","corrplot","sm","gapminder","openintro",
              ...
              "corrplot", "plotly","plumber")
ipak(packages)

#* @apiTitle API para leer archivo CSV, mostrar columnas y seleccionar una columna específica

#* Leer el archivo CSV y mostrar las columnas
#* @param file_path Ruta completa del archivo CSV
#* @param column_name Nombre de la columna a mostrar
#* @get /stats
function(file_path, column_name) {
  # Leer el archivo CSV
  data <- read.csv(file_path)
  
  # Verificar si la columna existe en los nombres de las columnas del dataframe
  if (!(column_name %in% colnames(data))) {
    return(list(
      error = TRUE,
      message = paste("La columna", column_name, "es inválida. Intente con un nombre de columna correcto.")
    ))
  }
  ...
  if (!is.numeric(column_data)) {
    return(list(
      error = TRUE,
      message = paste("La columna", column_name, "no es de tipo numérico. Por favor, seleccione una columna que contenga datos numéricos.")
    ))
  }
  ...
  stats <- list(
    ...
    quartiles = quantile(column_data, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)
  )
...
#* @apiTitle API para obtener la respuesta a la primera Hipotesis

#* ¿Existe una diferencia significativa en el salario mensual entre empleados de diferentes géneros en la misma posición y departamento?
#* @param file_path Ruta completa del archivo CSV
#* @get /fHipo
function(file_path) {
  # Leer el archivo CSV
...
#* @apiTitle API para obtener la respuesta a la segunda Hipotesis

#* ¿Cómo afecta el número de horas extras trabajadas al rendimiento (Job Rate) de los empleados en diferentes departamentos?
#* @param file_path Ruta completa del archivo CSV
#* @get /SHipo
function(file_path) {
  # Leer el archivo CSV
...
#* @apiTitle API para obtener la respuesta a la tercera Hipotesis

#* ¿Cuál es el impacto del número de años de servicio en la probabilidad de que un empleado tome licencias no remuneradas?
#* @param file_path Ruta completa del archivo CSV
#* @get /THipo
function(file_path) {
  # Leer el archivo CSV
  ...
  years_leave_prob <- employess_data %>%
    group_by(Years) %>%
    summarize(
      num_employees = n(),
...
#* @apiTitle API para obtener la respuesta a la cuarta Hipotesis
...
#* @apiTitle API para obtener la respuesta a la cuarta Hipotesis Segunda Parte
...
function(file_path) {
  ...
  htmlwidgets::saveWidget(fig_center,"fig_center.html")
  browseURL("fig_center.html")
  
}



#* @apiTitle API para obtener la respuesta a la quinta Hipotesis
